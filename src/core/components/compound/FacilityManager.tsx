...
const Page: React.FunctionComponent<PageProps> = () => {
  ...
  const top10ImportPlan = useQuery(
    ['top-10-import-plan'],
    async () => {
      const res = await importPlanApi.v1Get({
        filters: [`status||${FilterComparator.NOT_EQUAL}||DRAFT`],
        page: 0,
        orderBy: [`createdAt||${SortOrder.DESC}`],
        pageSize: 10,
  ...
  const top10ImportRequest = useQuery(
    ['top-10-import-request'],
    async () => {
      const res = await importRequestApi.v1Get({
        filters: [`status||${FilterComparator.NOT_EQUAL}||DRAFT`],
        page: 0,
        orderBy: [`createdAt||${SortOrder.DESC}`],
        pageSize: 10,
  ...
  return (
    <div className="grid grid-cols-2 gap-10 fade-in">
      ...
      <div className="flex flex-col gap-2 p-2 bg-white">
        <div className="font-bold text-LG">Kế hoạch mua sắm mới</div>
        <Table
          size="small"
          columns={[
            {
                ...
                return (
                  <FieldDisplay
                    ...
                    apiAction={async (data) => {
                      return NKRouter.importPlan.detail(data) as any;
                    }}
                  />
                );
              },
            ...
            },
            {
              ...
              render: (value, record) => {
                ...
                return <FieldDisplay value={status} type={FieldType.BADGE_API} apiAction={importPlanApi.v1GetEnumStatus} />;
              },
            },
          ]}
          dataSource={top10ImportPlan.data}
          pagination={false}
          footer={undefined}
        />
      </div>
      <div className="flex flex-col gap-2 p-2 bg-white">
        <div className="text-lg font-bold">Yêu cầu đặt thiết bị mới</div>
        <Table
          size="small"
          columns={[
            {
              ...
              render: (value, record) => {
                return (
                  <FieldDisplay
                    ...
                    type={FieldType.ROUTER}
                    apiAction={async (data) => {
                      return NKRouter.importRequest.detail(data) as any;
                    }}
                  />
                );
              },
...

            {
              ...
              key: 'expected',
              render: (value, record) => {
                const expected = _get(record, 'expected');
                return <FieldDisplay value={expected} type={FieldType.BADGE_API} apiAction={importRequestApi.v1GetEnumExpected} />;
              },
            },
            {
              title: 'Trạng thái',
              ...
              render: (value, record) => {
                const status = _get(record, 'status');
                return <FieldDisplay value={status} type={FieldType.BADGE_API} apiAction={importRequestApi.v1GetEnumStatus} />;
              },
            },
          ]}
          dataSource={top10ImportRequest.data}
          pagination={false}
          footer={undefined}
        />
      </div>
